# -*- coding: utf-8 -*-
"""Augment1+Weights.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vF_zmuVnA14p-7BuJK18iGk_CQtg_k6I
"""
...
df_train0['cell_type'].value_counts()

## Equalization sampling
df_train = df_train0
data_aug_rate = [3,2,0,10,0,0,10]
for i in range(7):
    if data_aug_rate[i]:
...
## Class weights
ytrain = df_train0["cell_type_idx"]
weight_for_0 = (1 / (ytrain == 0).sum())*(len(ytrain))/7.0 
weight_for_1 = (1 / (ytrain == 1).sum())*(len(ytrain))/7.0
weight_for_2 = (1 / (ytrain == 2).sum())*(len(ytrain))/7.0 
weight_for_3 = (1 / (ytrain == 3).sum())*(len(ytrain))/7.0
weight_for_4 = (1 / (ytrain == 4).sum())*(len(ytrain))/7.0 
weight_for_5 = (1 / (ytrain == 5).sum())*(len(ytrain))/7.0
weight_for_6 = (1 / (ytrain == 6).sum())*(len(ytrain))/7.0
...
history = model.fit(datagen.flow(x_train,y_train, batch_size = batch_size), epochs = epoch,
                              validation_data = (x_val, y_val), verbose = 1,
                                  ...
                                  callbacks=[cb_checkpointer],class_weight=class_weight)
